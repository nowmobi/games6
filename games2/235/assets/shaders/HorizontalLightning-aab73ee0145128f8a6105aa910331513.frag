...
void main() 
{
	...
	float part = floor(x / u_part_size);
	vec2 dispPartCoords = vec2(0.025 + u_dt / 10.0, u_part_size * part * u_random);
	vec2 dispNextPartCoords = vec2(0.025 + u_dt / 10.0, u_part_size * (part + 1.0) * u_random);
	...
	coords.y += (rand + (x - part * u_part_size) / u_part_size * (nextRand - rand)) * 0.025;
