...
fg_api.prototype.apstagModule = function () {
	...
	module.prototype.isEnabled = function() {
		return (
            ...
            !M.forceOff &&
            (self.config.is_own_inventory || 
                self.config.aid === "A-5U0J1" || 
                self.config.aid === "A-X95FH" || 
                self.config.aid === "A-SU86Z" || 
                self.config.aid === "A-DRAUGAS" || 
                self.config.aid === "A-ASNFC" || 
                self.config.aid === "A-CRAZYGAMES" || 
                self.config.aid === "A-SRFB8" || 
                self.config.aid === "A-MINIJUEGOS" || 
                self.config.aid === "A-GZD45" || 
                self.config.aid === "A-LIQPJ" || 
                self.config.aid === "A-TOS21" || 
                self.config.aid === "A-IGRICE-IGRICE" || 
                self.config.aid === "A-WPCLV" || 
                self.config.aid === "A-1CNRU" || 
                self.config.aid === "A-DF7EB")
		);
	};
...
fg_api.prototype.consentModule = function() {
    ...
    module.prototype.setupTcfApiListener = function() {
        __tcfapi('addEventListener', 2, function(tcData, success){
            console.log(
                "TCFv2 CMP API event:", tcData ? tcData.eventStatus : "no event data",
                "success:",	success,
                "data", tcData 
            );
			if (success) {
				...
				console.log("TCFv2 CMP API: failed to register listener");
			}
        });
    };
