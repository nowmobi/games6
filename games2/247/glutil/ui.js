var mizhu = new function () {
	...
	this.open = function (width, height, title, url, closed) {
		...
		var jq = $(html);
		jq.find(".window-panel").height(mizhu.height).width(mizhu.width).css("margin-left", -mizhu.width / 2).css("margin-top", -mizhu.height / 2);
	...
	this.cdmask = function (message, time) {
		var html = '<div class="win"><div class="mask-layer"></div></div>';
		var jq = $(html);
		jq.find(".window-panel").height(mizhu.height).width(mizhu.width).css("margin-left", -mizhu.width / 2).css("margin-top", -mizhu.height / 2);
		// jq.find(".title").find(":header").html(title);
		// jq.find(".body-panel").height(height - 36).attr("src", url);
	...
	var loadingJQ = null;
	this.loading = function (message, time) {
		var html = '<div class="win"><div class="mask-layer"><img class="circle" src="glutil/images/loading.png" alt="" /></div></div>';
		var jq = $(html);
		loadingJQ = jq;
		var circle_size = window.innerWidth / 4;
		if (window.innerWidth > window.innerHeight) circle_size = window.innerHeight / 4;
		jq.find('.circle').height(circle_size).width(circle_size).css("margin-top", window.innerHeight / 2 - circle_size).css("margin-left", window.innerWidth / 2 - circle_size / 2);
		jq.appendTo('body').fadeIn("fast");
	...
	this.hideloading = function () {
		loadingJQ.find('.mask-layer').hide();
	}
};
